...
sweepResults = lamb.sweeps.runParametricSweep(baseParams, options, sweepSpec, ...
...
if logical(p.Results.WriteOutputs)
    ...
    outputFolder = writeSensitivityOutputs(p.Results.LaunchFolder, ...
        "rayleigh_lamb", caseInfo.taskName, caseInfo.taskName + "_" + branchName, ...
    ...
    if strlength(scriptFile) > 0
        figureFolder = saveStudyFigure(fig, scriptFile, ...
