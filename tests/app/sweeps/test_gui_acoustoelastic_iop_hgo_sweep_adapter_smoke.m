...
for i = 1:numel(sweepOutput.normalized.curves)
    ...
    assert(isequal(curve.validMask(:), rawResult.validMask(:)), 'AE normalized validMask must mirror the canonical result.');
    ...
    if isfield(rawResult, 'fallbackCandidateCp')
        ...
        assert(all(~rawResult.validMask), 'Fallback-invalidated result must not expose official valid points.');
