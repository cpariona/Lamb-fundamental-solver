...
for iCase = 1:numel(cases)
    ...
    assert(raw.modelResult.execution.effectivePreset == "fast", 'Fit objective must use fast preset.');
    ...
    dense = fitOutput.normalized.fullCurve.denseSolver;
    assert(isfield(dense, 'rawResult') && isfield(dense.rawResult, 'modelResult'), ...
        'Full-curve solver diagnostic must preserve public model result.');
    assert(dense.rawResult.evaluationPath.usedPublicSolver == true, ...
        'Full-curve solver diagnostic must use public solver.');
    assert(dense.rawResult.modelResult.execution.effectivePreset == raw.modelResult.execution.effectivePreset, ...
        'Full curve must use the same effective preset as objective evaluation.');
    assert(dense.rawResult.modelResult.fallback.applied == false, ...
        'Full curve must not apply fallback.');
    ...
    if c.freeParam == "mu"
        assert(abs(requestedCurve.rawResult.modelResult.configuration.parameters.mu_Pa - fitResult.bestParams.mu) <= ...
            eps(max(1, fitResult.bestParams.mu)), ...
            'Requested fitted curve must use final fitted mu.');
    else
        assert(abs(requestedCurve.rawResult.modelResult.configuration.parameters.etaS_Pas - fitResult.bestParams.etaS) <= ...
