...
expectedAeResult = lamb.models.acoustoelastic_iop_hgo.aeSolveBranch(expectedAeParams, aeRaw.options);
...
assert(string(aeOutput.executionProfile.requestedExecutionProfile) == "Fast", ...
    'AE FitTool requested execution-profile metadata changed.');
assert(string(aeOutput.executionProfile.surfaceDefaultExecutionProfile) == "Fast", ...
    'AE FitTool surface-default execution-profile metadata changed.');
assert(nnz(string(aeOutput.normalized.parameterSummaryTable.Role) == "Fit") == 1, ...
    'AE FitTool parameter-summary rule changed.');
assert(isfield(aeOutput, 'fitElapsedSeconds') && isfinite(aeOutput.fitElapsedSeconds), ...
