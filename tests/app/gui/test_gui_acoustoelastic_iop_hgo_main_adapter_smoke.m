...
assert(isequal(fieldnames(result.metadata), {'params'; 'options'; 'modelResult'; ...
...
assert(isfield(result, 'metadata') && isfield(result.metadata, 'modelResult'), 'AE adapter must preserve modelResult.');
...
assert(isfield(result.metadata.modelResult, 'validMask'), 'AE model result must include validMask.');
assert(any(result.metadata.modelResult.validMask), 'AE model result must contain at least one valid Cp point.');
assert(isfield(result.metadata.modelResult, 'trackingFrequency'), 'AE model result must include internal tracking frequency.');
assert(numel(result.metadata.modelResult.trackingFrequency) >= numel(result.frequency), ...
...
assert(isequaln(result.metadata.modelResult.phaseVelocity_mps, expectedRawResult.phaseVelocity_mps), ...
...
assert(isequal(result.branches.phaseVelocity, result.metadata.modelResult.phaseVelocity_mps(:)), ...
    'AE Main GUI branch normalization must consume canonical Cp directly.');
assert(isequal(result.branches.diagnostics.valid, result.metadata.modelResult.validMask), ...
