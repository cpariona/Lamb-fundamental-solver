...
assert(isempty(csvPaths), 'Generated CSV files must not be versioned: %s', ...
    strjoin(csvPaths, ', '));
assert(~any(endsWith(lower(paths), "test_runtime_measurements.csv")), ...
    'Runtime measurements must remain ignored generated evidence.');
