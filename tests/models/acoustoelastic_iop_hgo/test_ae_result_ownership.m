...
modelRoot = fullfile(repositoryRoot, 'src', '+lamb', '+models', '+acoustoelastic_iop_hgo');

assertPath(which('lamb.models.acoustoelastic_iop_hgo.results.aeBuildResult'), ...
    fullfile(modelRoot, '+results', 'aeBuildResult.m'), 'canonical AE result builder');
assertPath(which('lamb.models.acoustoelastic_iop_hgo.quality.aeEvaluateAtlasA0Quality'), ...
    fullfile(modelRoot, '+quality', 'aeEvaluateAtlasA0Quality.m'), ...
    'canonical AE quality evaluator');
assertPath(which('lamb.models.acoustoelastic_iop_hgo.diagnostics.aeBuildIdentityA0DiagnosticBranch'), ...
    fullfile(modelRoot, '+diagnostics', 'aeBuildIdentityA0DiagnosticBranch.m'), ...
    'diagnostic identity extension');
assertPath(which('lamb.models.acoustoelastic_iop_hgo.diagnostics.aeScoreBranchIdentityCandidates'), ...
    fullfile(modelRoot, '+diagnostics', 'aeScoreBranchIdentityCandidates.m'), ...
    'diagnostic identity scorer');
assertPath(which('lamb.models.acoustoelastic_iop_hgo.solveAcoustoelasticIOPHGOBranch'), ...
    fullfile(modelRoot, 'solveAcoustoelasticIOPHGOBranch.m'), ...
...
ownerFiles = [ ...
    string(fullfile(modelRoot, 'solveAcoustoelasticIOPHGOBranch.m')); ...
...
assert(~contains(ownerText, "result.quality ="), ...
    'Solver/API owners must not assemble quality outside lamb.models.acoustoelastic_iop_hgo.results.aeBuildResult.');
...
result = lamb.models.acoustoelastic_iop_hgo.solvers.solveAcoustoelasticAtlasBranch(params, options);
fields = rmfield(result, {'quality', 'diagnostics', 'model', 'branch', ...
    'configuration', 'execution', 'wavenumber_radpm'});
rebuilt = lamb.models.acoustoelastic_iop_hgo.results.aeBuildResult(struct('fields', fields));
