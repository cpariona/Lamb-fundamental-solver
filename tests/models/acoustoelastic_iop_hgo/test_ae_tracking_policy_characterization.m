...
overrideResult = lamb.models.acoustoelastic_iop_hgo.aeSolveBranch(params, overrideOptions);
...
fallbackResult = lamb.models.acoustoelastic_iop_hgo.aeSolveBranch(fallbackParams, fallbackOptions);
...
refined = lamb.models.acoustoelastic_iop_hgo.aeSolveBranch(params, options);
options.refineLocalMinima = false;
discrete = lamb.models.acoustoelastic_iop_hgo.aeSolveBranch(params, options);
...
[a,b,g] = lamb.models.acoustoelastic_iop_hgo.constitutive.aeComputeABGFromIOPHGO(params.IOP, params.R, ...
    params.thickness, params.mu, params.k1, params.k2);
for j = 1:numel(params.frequency)
    objective = lamb.models.acoustoelastic_iop_hgo.core.aeObjectiveResidual(a,b,g,params.thickness, ...
...
tight = lamb.models.acoustoelastic_iop_hgo.aeSolveBranch(params, options);
...
if isempty(result.minimaTable), return; end
...
assert(isequal(result.selectedBranchPoints.Frequency_Hz, sort(result.selectedBranchPoints.Frequency_Hz)));
selectedRow = result.branchTable.BranchID == result.selectedBranchID;
assert(nnz(selectedRow) == 1);
assert(result.branchTable.SelectionScore(selectedRow) == min(result.branchTable.SelectionScore));
...
if isempty(result.selectedBranchPoints), return; end
for row = 1:height(result.selectedBranchPoints)
    ...
    if isempty(outputIndex), continue; end
    if ~result.validMask(outputIndex) || ~result.branchExistsAtFrequency(outputIndex), continue; end
