...
%% Local sensitivity must not shrink its mask when a perturbation loses coverage.
...
assert(all(isnan(S(:,1))), ...
    'Sensitivity must remain unavailable when either perturbation loses objective coverage.');
assert(~sensitivityInfo.coverageAccepted);
assert(~sensitivityInfo.parameterCoverageAccepted(1));
assert(contains(sensitivityInfo.coverageFailureReason(1), "negative:"));
assert(isequal(sensitivityInfo.objectiveMask, sensitivityExperimental.validMask));
