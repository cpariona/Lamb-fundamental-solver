...
normalized = lamb.fitting.normalizeExperimentalDispersionData(experimental);
...
validated = lamb.fitting.validateExperimentalDispersionData(experimental, 3);
assert(validated.numValidPoints == 3, 'Validation should preserve valid point count.');

CpModel_mps = [1.1 1.4 2.2 2.7];
[residuals, residualInfo] = lamb.fitting.computeDispersionFitResiduals(CpModel_mps, experimental);
...
[weightedResiduals, weightedInfo] = lamb.fitting.computeDispersionFitResiduals(CpModel_mps, experimental, weightedOptions);
...
metrics = lamb.fitting.computeDispersionFitMetrics(CpModel_mps, experimental);
...
[x, parameterInfo] = lamb.fitting.buildParameterVector(params, ["mu", "thickness"]);
...
updatedParams = lamb.fitting.unpackParameterVector([75e3; 600e-6], params, ["mu", "thickness"]);
...
[S, sensitivityInfo] = lamb.fitting.estimateLocalSensitivity(evaluateFcn, baseParams, ["a", "b"], normalized);
...
identifiability = lamb.fitting.assessFitIdentifiability(S, ["a", "b"]);
