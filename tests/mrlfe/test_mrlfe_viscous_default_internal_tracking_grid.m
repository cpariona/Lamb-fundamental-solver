...
assert(isfield(branch, 'internalTracking') && branch.internalTracking.used, ...
    'Viscous default branch must use internal tracking grid.');
assert(results.models.mRLFERealK.diagnostics.usedInternalTrackingGrid, ...
    'Viscous default diagnostics must report internal tracking grid usage.');

fprintf('test_mrlfe_viscous_default_internal_tracking_grid passed. etaS > 0 uses internal grid by default.\n');
