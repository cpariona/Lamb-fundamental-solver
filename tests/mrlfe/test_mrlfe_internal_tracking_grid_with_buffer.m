...
assert(isfield(branch, 'internalTracking') && branch.internalTracking.used, ...
    'Buffered viscous branch must report internal tracking grid usage.');
assert(numel(branch.internalTracking.trackingFrequency) > numel(requestedFrequency), ...
    'Buffered viscous internal tracking grid must be denser than requested grid.');
...
assert(viscousResults.models.mRLFERealK.diagnostics.usedInternalTrackingGrid, ...
    'Buffered viscous diagnostics must report internal tracking grid usage.');
assert(viscousResults.models.mRLFERealK.diagnostics.trackingPointCount > viscousResults.models.mRLFERealK.diagnostics.requestedPointCount, ...
    'Buffered viscous diagnostics must report a denser tracking grid.');

fprintf('test_mrlfe_internal_tracking_grid_with_buffer passed. Internal grid works with etaS > 0 buffer.\n');
