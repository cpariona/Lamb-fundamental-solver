...
if ~tracker.refineCandidates
    best = lamb.models.mrlfe.tracking.mrlfeRefineSelectedCandidate(best, CpScan, center, omega, material, geometry, mrlfeParams, tracker);
end
...
for i = 1:numel(CpScan)
    cp = CpScan(i);
    if isfinite(cp) && cp > 0
        residual(i) = lamb.models.mrlfe.core.mrlfeResidual(omega / cp, omega, material, geometry, mrlfeParams);
    end
end
end
