function result = aeSolveAtlasBranch(params, options)
%AESOLVEATLASBRANCH Track and refine a persistent atlas branch.
%
...
lamb.models.acoustoelastic_iop_hgo.configuration.aeValidateRequest(params, 'Context', "directAtlas");
timerStart = tic;

frequency = params.frequency(:).';
[objectiveMap, yGrid, cGrid, cShear] = lamb.models.acoustoelastic_iop_hgo.solvers.aeBuildAtlas(params, options);
rows = [];

for k = 1:numel(frequency)
    f = frequency(k);
    minima = lamb.models.acoustoelastic_iop_hgo.tracking.aeFindAtlasLocalMinima(cGrid, objectiveMap(:,k), cShear, options.atlasTopNMinima);
...
if isempty(rows)
    ...
    [minimaTable, branchTable] = lamb.models.acoustoelastic_iop_hgo.tracking.aeLinkAtlasBranches(minimaTable, options);
end
...
if strcmpi(string(options.atlasBranchPolicy), "identityA0Diagnostic")
    identity = lamb.models.acoustoelastic_iop_hgo.diagnostics.aeBuildIdentityA0DiagnosticBranch(result);
    spec = struct();
