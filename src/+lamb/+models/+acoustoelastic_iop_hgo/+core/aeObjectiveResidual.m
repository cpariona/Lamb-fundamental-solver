function [objectiveValue, details] = aeObjectiveResidual(alpha, beta, gamma, h, rho, rhoF, fluidBulkModulus, f, c, options, cpState)
%AEOBJECTIVERESIDUAL Objective for Li 2024 secular equation.
%
...
if nargout < 2
    M = lamb.models.acoustoelastic_iop_hgo.core.aeBuildMatrix(alpha, beta, gamma, h, rho, rhoF, fluidBulkModulus, f, c, options, cpState);
    [~, S, ~] = svd(M);
    sigmaMin = min(diag(S));
...
[M, aux] = lamb.models.acoustoelastic_iop_hgo.core.aeBuildMatrix(alpha, beta, gamma, h, rho, rhoF, fluidBulkModulus, f, c, options, cpState);
[U, S, V] = svd(M);
