...
%MRLFEBUILDSWEEPSOLVEREQUEST Map one mRLFE sweep point to the public request.
%
% This helper only translates SweepTool/app-layer sweep state into the public
% mrlfeSolve contract. It does not solve, plot, aggregate, or call GUI code.
...
if ~(branchName == "A0Like" || branchName == "S0Like")
    error('mrlfe:InvalidBranch', 'Unsupported mRLFE sweep branch "%s".', branchName);
end
...
pointParams = applySweepPoint(baseParams, sweepPoint, sweepOptions);
...
request.material = struct( ...
    'mu_Pa', getScalarAlias(pointParams, ["mu_Pa", "mu"], defaults.mu_Pa, 'mrlfe:InvalidMaterial', 'mu'), ...
    'etaS_Pas', getEtaS(pointParams, sweepOptions, defaults.etaS_Pas), ...
    'rho_kgm3', getScalarAlias(pointParams, ["rho_kgm3", "rho"], defaults.rho_kgm3, 'mrlfe:InvalidMaterial', 'rho'), ...
    'nu', getScalarAlias(pointParams, "nu", defaults.nu, 'mrlfe:InvalidMaterial', 'nu'));
request.geometry = struct( ...
    'thickness_m', getScalarAlias(pointParams, ["thickness_m", "thickness"], defaults.thickness_m, 'mrlfe:InvalidGeometry', 'thickness'));
request.fluid = struct( ...
    'density_kgm3', getFluidScalar(pointParams, sweepOptions, ["fluidDensity_kgm3", "fluidDensity", "density_kgm3"], ...
    "fluidDensity", defaults.fluidDensity_kgm3, 'fluid density'), ...
    'soundSpeed_mps', getFluidScalar(pointParams, sweepOptions, ["fluidSoundSpeed_mps", "fluidSoundSpeed", "soundSpeed_mps"], ...
...
function params = applySweepPoint(baseParams, sweepPoint, options)
params = baseParams;
if ~isstruct(sweepPoint) || ~isfield(sweepPoint, 'parameterName') || isempty(sweepPoint.parameterName)
    return;
end

parameterName = char(string(sweepPoint.parameterName));
...
if (parameterName == "fluidDensity" || parameterName == "fluidDensity_kgm3" || parameterName == "density_kgm3") && ...
        isstruct(options) && isfield(options, 'mrlfeParams')
    options.mrlfeParams.fluidDensity = value; %#ok<NASGU>
end
end
