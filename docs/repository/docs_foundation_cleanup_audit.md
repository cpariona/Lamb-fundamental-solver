...
- Clarified `docs/workflows/fitting/validation_suite.md` so mRLFE etaS fitting remains listed as a current validation limitation, not as a maintained synthetic parameter-recovery case.

## Resolved in pass 3

- Refreshed `docs/workflows/gui/integration_audit.md` as the active overview for `LambFundamental_GUI`, `SweepTool_GUI`, and `FitTool_GUI`.
...
- Reframed `docs/workflows/gui/main_pending_cleanup.md` as an active cleanup roadmap rather than a post-AE temporary cleanup note.
- Kept unresolved AE branch-sensitivity and layout items as roadmap items rather than deleting them.

## Resolved in pass 4

- Refreshed `docs/workflows/gui/adapter_architecture.md` as the detailed GUI/backend boundary reference.
...
- Reviewed `docs/models/rayleigh_lamb/overview.md`, `docs/models/rayleigh_lamb/public_api.md`, and `docs/models/rayleigh_lamb/fitting_workflow.md` against current tests and runner behavior.
...
- Corrected the naming convention path in AE framework hygiene status to `docs/models/acoustoelastic_iop_hgo/active/naming_and_paths_convention.md`.
