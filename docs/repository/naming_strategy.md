...
Retained implementation: `diagnose_idA0_plausibility_impl.m`
```
...
`diagnose_idA0_plausibility_impl` requires the workspace generated by
`validate_acoustoelastic_iop_hgo_identityA0_diagnostic_grid`; it is not a
standalone smoke-test command.

`diagnose_acoustoelastic_iop_hgo_modal_atlas` starts at low frequency by
design. Phase 1 removed simple forwarding aliases; the retained implementation
names are not renamed in this phase.
