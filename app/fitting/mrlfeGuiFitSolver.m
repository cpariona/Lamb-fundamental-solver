...
if strlength(branchName) == 0, branchName = "A0Like"; end
...
fitOutput = struct('request', request, 'modelFamily', "mrlfe", 'modelName', "mRLFERealK", ...
...
defaults = struct('etaS', 0.05, 'fluidDensity', 1000, 'fluidSoundSpeed', 1500, 'mrlfeA0Policy', "physicalTail");
for name = string(fieldnames(defaults)).'
    fieldName = char(name);
    if ~isfield(controls, fieldName) || isempty(controls.(fieldName)), controls.(fieldName) = defaults.(fieldName); end
end
end
function forwardModel = forwardModelPolicy(fitOptions)
forwardModel = struct('gridPolicy', "fitOptimized", 'minimumPointCount', 12, 'maximumPointCount', 40, 'maximumStep_Hz', 250);
if isstruct(fitOptions) && isfield(fitOptions, 'forwardModel') && isstruct(fitOptions.forwardModel)
    for name = string(fieldnames(fitOptions.forwardModel)).'
        fieldName = char(name); forwardModel.(fieldName) = fitOptions.forwardModel.(fieldName);
    end
end
end
function policy = buildRoutePolicy(branchName, freeParams, controls, evaluation, etaS)
policy = struct('branchName', string(branchName), 'freeParams', string(freeParams(:)), ...
    ...
    'actualPath', string(evaluation.path), 'mrlfeA0Policy', string(controls.mrlfeA0Policy), ...
    'etaS', etaS, 'fitAtlasPreset', string(evaluation.fitAtlasPreset), ...
    'forwardGridPolicy', string(evaluation.gridPolicy));
end
function policy = terminationPolicy(branchName)
if branchName == "A0Like", policy = "physicalTail"; else, policy = "none"; end
end
