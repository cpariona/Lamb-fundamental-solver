...
if strlength(branchName) == 0, branchName = "atlasA0"; end
branchName = lamb.models.acoustoelastic_iop_hgo.configuration.aeNormalizeBranchPolicy(branchName);
if branchName ~= "atlasA0", error('AE IOP/HGO fitting supports only atlasA0.'); end
controls = request.controls;
overrides = copyControlOverrides(struct(), controls, {'atlasNumYPoints', 'atlasTopNMinima', 'atlasInitializationNumFrequencyPoints'});
[solverOptions, profileMetadata] = aeResolveExecutionProfile(controls, ...
    'DefaultProfile', "Fast", 'DefaultSource', "FitTool default", ...
    'Surface', "FitTool", 'Overrides', overrides, ...
...
fitOutput = struct('request', request, 'modelFamily', "acoustoelastic_iop_hgo", ...
    'modelName', "AcoustoelasticIOPHGO", 'branchName', branchName, ...
    'fitResult', fitResult, 'normalized', normalized, 'executionProfile', profileMetadata, ...
    'fitElapsedSeconds', fitElapsedSeconds);
end
function target = copyControlOverrides(target, controls, names)
for i = 1:numel(names)
    name = names{i};
    if isfield(controls, name) && ~isempty(controls.(name)), target.(name) = controls.(name); end
end
end
