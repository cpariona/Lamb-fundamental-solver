...
if isempty(allShiftRows)
    mRLFEElasticHanCpShiftTable = table();
else
    mRLFEElasticHanCpShiftTable = struct2table(allShiftRows);
end

if isempty(summaryRows)
    mRLFEElasticHanCpShiftSummary = table();
else
    mRLFEElasticHanCpShiftSummary = struct2table(summaryRows);
end

if isempty(sampleRows)
    mRLFEElasticHanCpShiftSamples = table();
else
    mRLFEElasticHanCpShiftSamples = struct2table(sampleRows);
end

writetable(mRLFEElasticHanCpShiftTable, 'mRLFE_elastic_han_cp_shift_table.csv');
writetable(mRLFEElasticHanCpShiftSummary, 'mRLFE_elastic_han_cp_shift_summary.csv');
writetable(mRLFEElasticHanCpShiftSamples, 'mRLFE_elastic_han_cp_shift_samples.csv');
...
assignin('base', 'mRLFEElasticHanCpShiftTable', mRLFEElasticHanCpShiftTable);
assignin('base', 'mRLFEElasticHanCpShiftSummary', mRLFEElasticHanCpShiftSummary);
assignin('base', 'mRLFEElasticHanCpShiftSamples', mRLFEElasticHanCpShiftSamples);

fprintf('\nExported to workspace:\n');
fprintf('  mRLFEElasticHanCpComparisonResults\n');
fprintf('  mRLFEElasticHanCpComparisonEtaS\n');
fprintf('  mRLFEElasticHanCpShiftTable\n');
fprintf('  mRLFEElasticHanCpShiftSummary\n');
fprintf('  mRLFEElasticHanCpShiftSamples\n');
fprintf('Wrote CSV files:\n');
fprintf('  mRLFE_elastic_han_cp_shift_table.csv\n');
fprintf('  mRLFE_elastic_han_cp_shift_summary.csv\n');
fprintf('  mRLFE_elastic_han_cp_shift_samples.csv\n');
