% Stress-test Han-style viscoelastic real-k mRLFE stability.
%
% This diagnostic checks whether the Han-style Kelvin-Voigt shear-viscosity
% model remains trackable over practical elastic stiffness and viscosity
% ranges.  It uses the current elastic mRLFE real-k solution as reference;
% therefore A0-like elastic branch tracking benefits from the multicandidate
% DP tracker before the viscoelastic branch is solved.
%
% Model:
%   mRLFEHanViscoRealK
%   lambda real
...
%
% Notes:
%   - This is a validation/stress-test script, not a fitting script.
%   - The sweep is intentionally moderate to keep runtime manageable.
%   - Increase EValues, etaSValues, and numFrequencyPoints only after the
%     reduced range passes.

startup();

% Reduced but representative range for first-pass validation.
...
fprintf('\nmRLFE Han viscoelastic real-k stability sweep\n');
fprintf('----------------------------------------------\n');
...
for iE = 1:numel(EValues)
    ...
    for iEta = 1:numel(etaSValues)
        ...
        try
            ...
            fprintf('    ERROR: %s\n', ME.message);
            summaryRows = [summaryRows; makeFailedRow('A0Like', params, material, etaS, ME.message)]; %#ok<AGROW>
            summaryRows = [summaryRows; makeFailedRow('S0Like', params, material, etaS, ME.message)]; %#ok<AGROW>
        end
    end
end

mRLFEHanViscoRangeStabilitySummary = rowsToTable(summaryRows);
writetable(mRLFEHanViscoRangeStabilitySummary, 'mRLFE_han_visco_range_stability_summary.csv');

assignin('base', 'mRLFEHanViscoRangeStabilityResults', resultsByCase);
assignin('base', 'mRLFEHanViscoRangeStabilitySummary', mRLFEHanViscoRangeStabilitySummary);
assignin('base', 'mRLFEHanViscoRangeStabilityEValues', EValues);
assignin('base', 'mRLFEHanViscoRangeStabilityEtaSValues', etaSValues);
assignin('base', 'mRLFEHanViscoRangeStabilityLargeJumpThreshold', largeJumpThreshold);

fprintf('\nHan viscoelastic range stability summary\n');
fprintf('----------------------------------------\n');
if ~isempty(mRLFEHanViscoRangeStabilitySummary)
    disp(mRLFEHanViscoRangeStabilitySummary(:, {'Branch','E_kPa','EtaS_Pa_s','ValidPoints','TotalPoints','ValidFmax_Hz','SafeFmax_Hz','MaxResidual','MaxRelativeCpJump','FirstLargeJumpRelative','HasWarning'}));
end
fprintf('\nWrote mRLFE_han_visco_range_stability_summary.csv\n');

% Compact visual summaries.
plotSafeFmaxSummary(mRLFEHanViscoRangeStabilitySummary, 'A0Like');
plotSafeFmaxSummary(mRLFEHanViscoRangeStabilitySummary, 'S0Like');
...
xlabel('E [kPa]');
ylabel('Safe fmax [Hz]');
title(sprintf('Han viscoelastic real-k %s safe fmax', branchName));
