% Stress-test real-k mRLFE stability over stiffness and shear-viscosity ranges.
%
% This diagnostic checks whether the fluid-loaded real-k mRLFE branch remains
% trackable over practical elastic stiffness values and Kelvin-Voigt shear
% viscosities.
%
% Model convention:
...
%
% Output file:
%   mRLFE_real_k_range_stability_summary.csv
...
mRLFERealKRangeStabilitySummary = rowsToTable(summaryRows);
writetable(mRLFERealKRangeStabilitySummary, 'mRLFE_real_k_range_stability_summary.csv');

assignin('base', 'mRLFERealKRangeStabilityResults', resultsByCase);
assignin('base', 'mRLFERealKRangeStabilitySummary', mRLFERealKRangeStabilitySummary);
assignin('base', 'mRLFERealKRangeStabilityEValues', EValues);
assignin('base', 'mRLFERealKRangeStabilityEtaSValues', etaSValues);
assignin('base', 'mRLFERealKRangeStabilityLargeJumpThreshold', largeJumpThreshold);

fprintf('\nmRLFE real-k range stability summary\n');
fprintf('-----------------------------------\n');
if ~isempty(mRLFERealKRangeStabilitySummary)
    disp(mRLFERealKRangeStabilitySummary(:, {'Model','Branch','E_kPa','Mu_kPa','EtaS_Pa_s','ValidPoints','TotalPoints','ValidFmax_Hz','SafeFmax_Hz','MaxResidual','MaxRelativeCpJump','FirstLargeJumpRelative','HasWarning'}));
end
...
ylabel('Safe fmax [Hz]');
title(sprintf('mRLFE real-k %s safe fmax', branchName));
